...
export function buildDynamics(game: Game, P: Planet): void {
  ...
  // ground-attached elements sit on the real terrain surface (steps/dunes/hills)
  const gAt = (x: number) => game.groundAt(x);
...
/** Gameplay-relevant motion, advanced once per fixed physics step (frame-rate
 *  independent, and holds still while paused): everything the blob can stand
 *  on or bump into — moving platforms, rollers, mover clouds/ice, bubbles. */
export function stepDynamics(game: Game, sp: number): void {
  ...
  game.rollers.forEach(r => { if (!r.alive) return; r.mesh.position.x += r.dir * 0.025 * sp * (calm ? 0.5 : 1); if (Math.abs(r.mesh.position.x - r.home) > r.range) r.dir *= -1; r.mesh.position.y = game.groundAt(r.mesh.position.x) + 0.45; if (r.mesh.userData.rock) r.mesh.userData.rock.rotation.z -= r.dir * 0.06 * sp; });
  game.movers.forEach(mv => { const amp = mv.amp || 0.5; mv.mesh.position.y = mv.baseY + Math.sin(t * 0.7 + mv.phase) * amp * (calm ? 0.4 : 1); if (mv.drift) { mv.mesh.position.x = mv.x + Math.sin(t * 0.3 + mv.phase) * 2.0; } });
  game.bubbles.forEach(bb => { bb.mesh.position.y = bb.y + Math.sin(t * 1.2 + bb.phase) * 0.3 * (calm ? 0.4 : 1); });
}

/** Purely visual animation (bobbing, drifting particles), run per render frame. */
export function updateDynamics(game: Game, sp: number): void {
  const calm = game.calm;
  const clock = game.stage.clock;
  game.movers.forEach(mv => { mv.mesh.rotation.y += 0.003 * sp; });
  game.bubbles.forEach(bb => { bb.mesh.userData.b.scale.setScalar(1 + Math.sin(clock.elapsedTime * 2 + bb.phase) * 0.06); });
